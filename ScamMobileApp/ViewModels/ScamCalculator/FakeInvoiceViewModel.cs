...
{
    public class FakeInvoiceViewModel : BaseViewModel
    {
        public FakeInvoiceViewModel(INavigation navigation)
        {
        ...
        private async Task FirstQuestionCommandExecute()
        {
            ...
            await Navigation.PushAsync(new FakeSecondQuestion());
        ...
        private async Task SecondQuestionCommandExecute()
        {
            ...
            await Navigation.PushAsync(new FakeThirdQuestion());
        ...
        private async Task ThirdQuestionCommandExecute()
        {
            ...
            await Navigation.PushAsync(new FakeFourthQuestion());
        ...
        private async Task ScamCalculatorCommandExecute()
        {
            try
            {
                ...
                if (newResult > 25)
                {
                    ...
                    Application.Current.MainPage = new NavigationPage(new FakeLikelyResult());

                    ScamResult = "The Q&A assessment strongly suggests a possible scam. If you get an unexpected invoice, verify by contacting the supplier directly. Avoid links or attachments that could be malware. For complaines: Train employees and use robust verification to prevent fake invoice scams. Stay cautious, verify, and report if necessary.";
                ...
                else
                {
                    ...
                    Application.Current.MainPage = new NavigationPage(new FakeUnlikelyResult());
        ...
        public async Task PostFeedbackCommandExecute()
        {
            try
            {
                ...
                string[] items = { "Is the invoice from a known and verified supplier or service provider that you have previously done business with? ",
                                    "Is the invoice for goods or services you didn't order or that seem unfamiliar?",
                                    "Does the invoice contain unusual or inconsistent information, such as incorrect contact details, grammar errors, or altered payment instructions?",
                                    "Are you being pressured to pay the invoice immediately or face consequences, such as penalties or legal action?"
                                };
                ...
                { questionAnswer = itemList, comment = UserComment, output = scamlikely, rating = newrating, outputDetails = updatedResult, scamType = "Fake Invoice scams" };
