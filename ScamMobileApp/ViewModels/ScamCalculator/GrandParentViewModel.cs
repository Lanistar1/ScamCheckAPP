...
{
    public class GrandParentViewModel : BaseViewModel
    {
        ...
        private async Task FirstQuestionCommandExecute()
        {
            ...
            await Navigation.PushAsync(new GrandParentSecondQuestion());
        ...
        private async Task SecondQuestionCommandExecute()
        {
            ...
            await Navigation.PushAsync(new GrandParentThirdQuestion());
        ...
        private async Task ThirdQuestionCommandExecute()
        {
            ...
            await Navigation.PushAsync(new GrandParentFourthQuestion());
        ...
        private async Task ScamCalculatorCommandExecute()
        {
            try
            {
                ...
                if (newResult > 25)
                {
                    ...
                    Application.Current.MainPage = new NavigationPage(new GrandParentLikelyResult());

                    ScamResult = "The Q&A assessment strongly suggests a possible scam. Scammers often use emotional tactics to pressure victims into quick decisions. If you receive a call from someone claiming to be a friend or family member in need, take a moment to assess the situation calmly. Verify their identity and the details of the situation before taking any action. Educate your family members about these scams to prevent them from falling victim.";
                ...
                else
                {
                    ...
                    Application.Current.MainPage = new NavigationPage(new GrandParentUnlikelyResult());
        ...
        public async Task PostFeedbackCommandExecute()
        {
            try
            {
                ...
                string[] items = { "Can you confirm the caller's identity by asking personal questions or mentioning specific family details that only they would know?",
                                    "Is the caller urging you to keep the situation secret and not discuss it with other family members?",
                                    "Is the caller requesting immediate payment or asking you to send funds using untraceable methods, such as gift cards or wire transfers?",
                                    "Can you reach out to other family members using known, reliable contact information to confirm the situation?"
                                };
                ...
                { questionAnswer = itemList, comment = UserComment, output = scamlikely, rating = newrating, outputDetails = updatedResult, scamType = "Grandparent or Parent Scam" };
